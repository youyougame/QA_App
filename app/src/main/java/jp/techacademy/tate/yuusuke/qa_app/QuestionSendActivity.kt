    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == CHOOSER_REQUEST_CODE) {
            ...
            try {
                ...
                image = BitmapFactory.decodeStream(inputStream) // Bitmapの暗号を復元する
                inputStream!!.close()
            } catch (e: Exception) {
                return
            }
        ...
        if (v === imageView) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                    // 許可されている
                    showChooser() // ギャラリーとカメラを選択するダイアログを表示
                } else {
            ...
            if (drawable != null) {
                ...
                val bitmapString = Base64.encodeToString(baos.toByteArray(), Base64.DEFAULT) // Bitmapを暗号化
